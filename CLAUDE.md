...
- `src/Agent/` — the panel agent. `PanelAssistant` (a laravel/ai agent, usable with no subclass) composes the knowledge tools and `ResourceToolRegistry`'s tools. A Filament resource opts in with `AgentResource` + `InteractsWithAgent`; `ResourceInspector` reads its table, form and labels into a `ResourceBlueprint`, and `ListRecordsTool` / `ViewRecordTool` act on that blueprint only. `CreateRecordTool` / `EditRecordTool` / `DeleteRecordTool` write through the form's fields -- `FormFieldMapper` maps them by `instanceof`, an unknown field class degrades to a string, a field that is not one attribute value is dropped -- and implement laravel/ai's `Approvable`.
...
- `src/Filament/Pages/AgentSettings.php`, `src/Agent/Resources/ResourcePolicies.php` — what the assistant may do, edited from the panel. The page writes `rag.agent.*` through the same `SettingsRepository` as the knowledge settings; `ResourcePolicies` applies `rag.agent.resources.overrides` on top of each resource's own `agentTools()`.
- `src/Agent/Chat/`, `src/Filament/Pages/AssistantChat.php` — the chat page. `ConversationTranscript` reads messages, tool outcomes and pending approvals straight from laravel/ai's conversation tables; `PageContext` turns the page on screen into a resource slug and record key for the topbar button; `AssistantAccess` is the on/off switch and the `rag.agent.authorize` callback.
...
- **Writes mirror Filament's own pages, not their hooks.** `new Model($data)` + `save()` and `$record->update($data)` are exactly `CreateRecord::handleRecordCreation()` and `EditRecord::handleRecordUpdate()`, run in a transaction, with only the form's field names taken from the arguments. Page hooks (`mutateFormDataBeforeCreate()` and friends) do not run -- they live on page instances the agent never mounts. `CreateRecord` itself contains no tenant association and there is no multi-tenant fixture yet: check tenant ownership on a tenant panel before relying on agent-created records there.
- **`PanelAssistant::provider()` and `model()` are what make `rag.llm.*` reach the agent.** laravel/ai asks an agent for those two methods before falling back to `config('ai.default')`, and `LaravelAiLanguageModel` is the *retrieval* pipeline's model, never the agent's. Without them a host with `RAG_LLM_PROVIDER=anthropic` had its panel agent call OpenAI with an empty key and get a 401 -- which is exactly how it was found, in the demo app. `AssistantProviderTest` fakes the gateway on one named provider, so a regression resolves elsewhere and fails.
...
- **Piston refuses a `run_timeout` above its own ceiling, 3s by default.** It answers 400 with a message rather than clamping, which the driver surfaces as an error the model can read; `PISTON_RUN_TIMEOUT` raises the ceiling and `rag.agent.sandbox.timeout` must stay under it. Same shape for output: Piston's own 1KB cap truncates before the application gets to choose what to keep.
- **Code execution is off unless a host switches it on, and every run is written to the log with its snippet.** "The agent computed it" is not an audit trail. `rag.agent.sandbox.enabled` gates the tool and `RunCode::enabled()` is what `PanelAssistant` asks before offering it.
- **`maxSteps()` is what makes a sandbox usable.** Write a program, run it, read the error, fix it: four round trips before an answer. `rag.agent.max_steps` feeds `PanelAssistant::maxSteps()`; null leaves laravel/ai's own default, which is lower.
- **The sandbox's url and driver are not editable from the panel.** Everything else about it is -- on/off, languages, time limit, output caps -- but a form that decides where the application posts code is an SSRF, not a setting, and whoever administers a panel is not necessarily whoever controls the network. `rag.settings.overridable` says so in a comment next to the keys that *are* there.
