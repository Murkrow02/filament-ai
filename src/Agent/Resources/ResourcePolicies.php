 ...
 *
 * They live in `rag.agent.resources.overrides`, which the settings repository
 * writes from the panel, so what the agent may do can be changed without a
 * deployment. The code keeps the last word on two things: a resource that
 * never opted in cannot be enabled from here, and a deletion is always
...
final class ResourcePolicies
{
    ...
    public static function apply(AgentTools $tools, string $resource): AgentTools
    {
        ...
        if (isset($override['unapproved']) && is_array($override['unapproved'])) {
            // AgentTools refuses to take approval off a deletion; filtering
            // here keeps a hand-edited config from throwing on every request.
            $unapproved = array_values(array_intersect(
                self::strings($override['unapproved']),
                [AgentTools::CREATE, AgentTools::EDIT],
            ));

            if ($unapproved !== []) {
                $tools = $tools->withoutApproval(...$unapproved);
            }
        }
